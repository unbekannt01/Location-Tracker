...
const setData = async (path, data) => {
  if (!db) throw new Error("Firebase not initialized. Check your environment variables.")
...
const getAdminByCredentials = async (username, password) => {
  ...
  const matchingEntry = Object.entries(users).find(
    ([id, user]) => user.username === username && user.password === password && user.role === "admin",
  )
...
export default function App() {
    ...
    if (userInfo && userInfo.type === "member") {
      const fetchMemberGroup = async () => {
        try {
          const users = await getData("users")
          if (users) {
            const user = Object.values(users).find((u) => u.username === userInfo.username && u.role === "member")
            if (user) {
              setGroupId(user.groupId)
              setUserInfo({ ...userInfo, memberId: user.id, groupId: user.groupId })
            }
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (groupId && isTracking) {
      const unsubscribe = onDataChange(`groups/${groupId}/locations`, (locations) => {
  ...
  const startBackgroundLocation = async (member) => {
    try {
      ...
      if (fgStatus !== "granted") {
        console.log("Foreground location permission denied for background task")
        return
      }
  ...
  const handleLogin = async (loginData) => {
    if (loginData.type === "admin") {
      try {
        setLoading(true)
        const admin = await getAdminByCredentials(loginData.username, loginData.password)
        if (admin) {
          setIsLoggedIn(true)
          setUserRole("admin")
      ...
      try {
        setLoading(true)
        const user = await getUserByCredentials(loginData.username, loginData.password)
        if (user) {
          if (!user.groupId) {
          ...
          setIsLoggedIn(true)
          setUserRole("member")
          const fullUserInfo = { ...loginData, memberId: user.id, groupId: user.groupId }
          setUserInfo(fullUserInfo)
          setGroupId(user.groupId)
          setCurrentScreen("home")
          await startBackgroundLocation({ id: user.id, groupId: user.groupId, username: user.username })
        } else {
  ...
  if (currentScreen === "createGroup") {
    return <CreateGroupScreen onGroupCreated={handleGroupCreated} onBack={handleBackToHome} />
  }
  ...
  if (currentScreen === "membersList") {
    return (
      <MembersListScreen
        ...
        onBack={userRole === "admin" ? handleBackToGroupsList : handleBackToHome}
  ...
  if (currentScreen === "groupsList") {
    return (
      <GroupsListScreen onSelectGroup={handleSelectGroup} onBack={handleBackToHome} onCreateGroup={handleCreateGroup} />
    )
  }
  ...
  if (currentScreen === "adminLocationTracking") {
    ...
    return <AdminLocationTrackingScreen groupId={groupId} onBack={handleBackToGroupsList} />
  }

  if (currentScreen === "changePassword") {
    return (
      <ChangePasswordScreen userInfo={userInfo} onBack={handleBackToHome} onPasswordChanged={handlePasswordChanged} />
    )
  }
